...
- **設定専用のデスクトップアプリ。** アプリはサインイン、モデル設定、MCP ガイド、ヘルスチェック、
  安全な診断、更新、修復を管理します。通常の自動ターンはヘッドレスバックエンドで実行し、
  内蔵ブラウザーはゼロリスク／手動互換パスだけに残します。
...
設定アプリをインストールまたは更新します。既存のインストールを更新・修復する場合は、
...
```bash
curl -fsSL https://github.com/kbwumin13-sudo/web-gpt/releases/latest/download/install-launcher.sh | sh
```

アプリ内で次の 3 項目を完了します。

1. 設定からサインインします。自動モードではヘッドレスバックエンドが使用する管理対象 Chrome プロファイルを開きます。
   ゼロリスクでは内蔵ブラウザーが明示的な手動互換パスとして残ります。
2. ゼロリスクを使う場合は、ブラウザーのスモークテストを実行します。
...
パッケージ版の設定アプリは自動モデルターンを管理しません。自動モードではヘッドレスバックエンドが
設定済みの Chrome 実行ファイルを使用し、アプリは要求時だけログイン処理を起動します。モデル API キー、
システムの Node/Bun、プロジェクト管理のブラウザーダウンロードは不要です。

**ソースから実行**

```bash
git clone https://github.com/kbwumin13-sudo/web-gpt.git && \
...
> [Limits](https://github.com/kbwumin13-sudo/web-gpt/discussions/309) を参照してください。
...
セットアップではローカルの `web_agent_runner` MCP server も登録されます。ネイティブ Codex
タスクから唯一の `web_agent_run` ツールを呼び出し、`chatgpt-web/light`、
`chatgpt-web/medium`、`chatgpt-web/high`、`chatgpt-web/extra-high`、または
`chatgpt-web/pro` を明示的に選択できます。`design` は読み取り専用の一時 App Server タスク、
`execute` は workspace-write で実行し、コマンドやファイル変更の承認を現在の Codex に返します。
別モデルへのフォールバックは行わず、子タスク内では自身の Runner を無効にします。

インストールされるエントリポイントは次のとおりです。
...
- 公開リリースパッケージは現在、macOS 13+（arm64/x64）のみを対象としています。内部 CI では Windows と Linux の互換性も検証します。
